...
var EnableTableGroup = "enableTableGroup"
...
function findChild(element, str, allowCache, returnAll) {
    var splitIdx = str.indexOf("::");
    if (splitIdx>=0) {
        var items = str.split("::");
        var parents = findChild(element, items[0], allowCache, true);
        if (!parents) return null;
        for (idx in parents) {
          var parent = parents[idx]
          var newStr = str.substring(splitIdx+2)
          result = findChild(parent, newStr, allowCache);
          if (result) {
            return result
          }
        }
    ...
    var allResults = []; // only for returnAll case
    for (var i in array) {
        var element = array[i];
        if (!inScreen(element)) {
            continue;
...
function performStep(step) {
    ...
    /**** Background  *********/
    else if (Background == action) {
      var duration = 1;
      if (step[1]) {
        duration = step[1];
      }
      UIATarget.localTarget().deactivateAppForDuration(duration);
    }

    else if (EnableTableGroup == action) {
      if (step.length > 1) {
        GlobalEnv.EnableTableGroup = step[1];
      }
    }
